...
const LectureQuiz = ({ quizIDPath, lecturerID }: { quizIDPath: string, lecturerID: string | undefined }) => {
  ...
  React.useEffect(() => {
    LecturerService.getQuizQuestionList(quizID).then(
      ({ data }: { data: QuizQuestionData[] }) => {
        setQuizQuestionList(data);
      }
    );
    LecturerService.getAttemptDetail(quizID).then(
